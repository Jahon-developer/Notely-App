    ...
    <div className="Container">
      ...
      {isData ? (
        <div className='cart_row'>
          {data.map((cart, id) => (
        <Cart setIsModal={setIsModal} key={id} isCompleted={cart.isCompleted} title={cart.title} text={cart.text} 
        date={cart.date} category={cart.category} />
      ))}
        </div>
      ) :  (
      <NotFound/>)}
      </main>
      {isModal.isActive ? <Modal isModal={isModal} setIsModal={setIsModal}/> : ""}
    </div>
  );
};
